...
{
    public partial class AddVatInvoice : Form
    {
        DataTable tempItems = new DataTable();
        decimal total = 0;
        decimal tax = 0;
        decimal taxableAmount = 0;
        decimal vat = Constants.Vat;
        bool vatBill = true;

        public AddVatInvoice()
        {
            InitializeComponent();
        }

        private void AddVatInvoice_Load(object sender, EventArgs e)
        {
            Logger.LogInfo("Enters AddVatInvoice_Load");
            try
            {
                BindCmbCustomer();
                lblVat.Text = "Vat " + vat + "% :";
        ...
        {
            Logger.LogInfo("Enters AddVatInvoice_btnAdd_Click");
            try
            {
                decimal qty = 0;
                decimal rate = 0;
            ...
            Logger.LogInfo("Exits AddVatInvoice_btnAdd_Click");
        }

        private void btnDelete_Click(object sender, EventArgs e)
        {
            Logger.LogInfo("Enters AddVatInvoice_btnDelete_Click");
            try
            {
            ...
            Logger.LogInfo("Exits AddVatInvoice_btnDelete_Click");
        }
        ...
        {
            Logger.LogInfo("Enters AddVatInvoice_btnSave_Click");
            try
            {
                if (total <= 0)
                {
                ...
                var invoiceId = blInvoice.InsertVatInvoice(cdoInvoice);
                ...
                if (invoiceId > 0)
                {
                    foreach (DataRow item in tempItems.Rows)
                    {
                        ...
                        var invoiceItemId = blInvoice.InsertVatInvoiceItem(cdoInvoiceItems);
                    }
            ...
            Logger.LogInfo("Exits AddVatInvoice_btnSave_Click");
        }
